...
# ) # import game logic
# from models.value_iteration import get_ai_move # import ai_move by the model you choose
#
#
...
# ai_module_name = "models.value_iteration"
...
def init_policy():
    """
    If your model needs an explicit init (e.g. value_iteration()),
    you can call it here so it doesn’t block at import time.
    """
...
def make_move():
    ...
    if is_player_A:
        ...
        # AI’s move: note the full signature includes scores
        pos, step = ai_module.get_ai_move(board, score_A, score_B, False)
        ai_move_description = (
            f"AI chose box [{pos}] and moved "
            f"{'clockwise' if step == 1 else 'counterclockwise'}"
        )

    # 1) Capture seeds
    captured_dan, captured_quan = capture(board, pos, is_player_A, step)
    target_score = (score_A if is_player_A else score_B)
    target_score["dan"]  += captured_dan
    target_score["quan"] += captured_quan

    # 2) Restore any leftover seeds onto the player’s side
    restore_player_side(board, target_score, is_player_A)
...
def switch_model():
    """
    Dynamically switch to another AI strategy under models/.
    The JSON body should be: { "model": "<module_name>" }
    """
    global ai_module
    model_name = request.json["model"]
    ai_module = importlib.import_module(f"models.{model_name}")
    # If the new module has an init_policy hook, call it
    if hasattr(ai_module, "init_policy"):
        ai_module.init_policy()
    return jsonify({"status": f"Model switched to {model_name}"})
