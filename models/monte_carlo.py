...
def ai_turn_monte_carlo(board, score, is_player_A, episode):
    state = board_to_state(board)
    move = get_ai_move(board, score)
    if move is None:
        return False
    pos, step = move
    ...
    episode.append((str(state), str(move), reward))
    return True


def monte_carlo_update(Q_table, episode, gamma=0.9):
    G = 0
    visited = set()
    for state, action, reward in reversed(episode):
        ...
        if (state, action) not in visited:
            ...
            Q_table[(state, action)] = old_value + 0.1 * (G - old_value)
...
#
#     if TRAINING_MODE:
#         for _ in range(10000):  # Train for 10,000 games
#             board = [["quan"], ["dan"] * 5, ["dan"] * 5, ["dan"] * 5, ["dan"] * 5, ["dan"] * 5,
#                      ["dan"] * 5, ["dan"] * 5, ["dan"] * 5, ["dan"] * 5, ["dan"] * 5, ["quan"]]
#             score_A = {"dan": 0, "quan": 0}
#             score_B = {"dan": 0, "quan": 0}
...
#                 ai_turn_monte_carlo(board, score_A, is_player_A=True, episode=episode)
#                 if game_over(board, score_A, score_B):
#                     break
#                 ai_turn_monte_carlo(board, score_B, is_player_A=False, episode=episode)
#
#             monte_carlo_update(Q_table, episode)
#
#         with open("q_table.json", "w") as f:
#             json.dump({str(k): v for k, v in Q_table.items()}, f)
# Turn on when human vs AI
episode = []  # Reset episode for each game
...
# After game ends, update Q-table using Monte Carlo
monte_carlo_update(Q_table, episode)
