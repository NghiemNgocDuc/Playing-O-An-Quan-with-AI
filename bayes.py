...
def get_ai_move(board, is_player_A):
    ...
    def evaluate_move(x):
        ...
        return -calculate_score(temp_score_B["dan"], temp_score_B["quan"])

    space = [
        (6, 10),  # Valid positions for Player B
        (-1, 1)  # Step: -1 (CCW), 1 (CW)
    ]
    ...
    best_pos = int(result.x[0])
    best_step = int(result.x[1])
    return best_pos, best_step
